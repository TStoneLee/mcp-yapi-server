 ...
 */
class YApiClient {
    constructor(baseUrl = process.env.YAPI_BASE_URL || 'https://yapi.example.com') {
     ...
     * 获取接口详情
     */
    async getInterfaceDetails(interfaceId, token) {
        try {
     ...
     * 获取接口列表
     */
    async getInterfaceList(projectId, catId, token) {
        try {
            let endpoint;
            const params = {};
            if (catId) {
     ...
     * 搜索接口
     */
    async searchInterface(projectId, keyword, token) {
        try {
            const params = {
...
class InterfaceFormatter {
     ...
     * 格式化接口详情为结构化数据
     */
    static formatInterfaceDetails(interfaceData) {
     ...
     * 提取请求参数
     */
    static extractRequestParams(interfaceData) {
        const params = [];
        // Query参数
        if (interfaceData.req_query && Array.isArray(interfaceData.req_query)) {
            interfaceData.req_query.forEach((param) => {
                params.push({
                    name: param.name,
                    location: 'query',
        ...
        if (interfaceData.req_params && Array.isArray(interfaceData.req_params)) {
            interfaceData.req_params.forEach((param) => {
                params.push({
                    name: param.name,
                    location: 'path',
                    required: true,
     ...
     * 提取请求头
     */
    static extractHeaders(interfaceData) {
        const headers = [];
        if (interfaceData.req_headers && Array.isArray(interfaceData.req_headers)) {
            interfaceData.req_headers.forEach((header) => {
                headers.push({
                    name: header.name,
                    value: header.value || '',
     ...
     * 提取请求体
     */
    static extractRequestBody(interfaceData) {
        const body = {
     ...
     * 提取响应信息
     */
    static extractResponseInfo(interfaceData) {
        const response = {
...
推荐用法: 直接粘贴YApi接口链接,如: https://yapi.example.com/project/100/interface/api/12345`,
    ...
    server.registerTool('yapi_get_interface_by_url', {
        ...
        try {
            ...
            try {
                const interfaceData = await yapiClient.getInterfaceDetails(parsed.interfaceId, args.token);
    ...
    // 注册工具1: 获取接口详情
    server.registerTool('yapi_get_interface', {
        ...
        try {
            const interfaceData = await yapiClient.getInterfaceDetails(args.interface_id, args.token);
            const formatted = InterfaceFormatter.formatInterfaceDetails(interfaceData);
            return {
                content: [
                    {
    ...
    // 注册工具2: 获取项目信息
    server.registerTool('yapi_get_project', {
        ...
        try {
            const projectInfo = await yapiClient.getProjectInfo(args.project_id, args.token);
            return {
                content: [
                    {
                        type: 'text',
    ...
    // 注册工具3: 获取接口列表
    server.registerTool('yapi_list_interfaces', {
        ...
        try {
            const interfaces = await yapiClient.getInterfaceList(args.project_id, args.cat_id, args.token);
            return {
                content: [
                    {
                        type: 'text',
    ...
    // 注册工具4: 搜索接口
    server.registerTool('yapi_search_interface', {
        ...
        try {
            const results = await yapiClient.searchInterface(args.project_id, args.keyword, args.token);
            return {
                content: [
                    {
                        type: 'text',
